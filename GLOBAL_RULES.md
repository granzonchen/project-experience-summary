...
| ⑦ 多宿主 | 纪律**与宿主无关**（护栏不关心推哪儿）；脚本**参数化远端**（`--remote=`/`--branch=`）；各宿主由同一棵树分别 orphan 重建（**提交 SHA 不同、树 SHA 相同**），**每个宿主都要推**，复验比对树 SHA 是否相等 |
| ⑧ 可见性判定 | **必须"真匿名"**：直接 `git clone/ls-remote` 会被本机**凭据助手自动补凭据**，私有仓也"克隆成功"→ 误判为**公开**（2026-09-12 实际误判一次）。修法：`HOME`/`USERPROFILE`→空目录 + `GIT_CONFIG_NOSYSTEM=1` + 禁交互；并用平台 API 匿名 404 交叉验证 |
...
[ ] 多宿主各自推同一棵树（比对树 SHA 一致）；可见性用**真匿名**方式判定（禁凭据助手），不以 git 试探为准（R-26）
```
